    ...
    : QWidget(parent), repository_(repository)
{
    ...
    auto *import = new QPushButton(QStringLiteral("导入 WakeUp 备份"), this);
    import->setObjectName(QStringLiteral("importWakeUp"));
    ...
    connect(import, &QPushButton::clicked, this, &CourseSidebar::importWakeUp);
...
void CourseSidebar::refreshCourses()
{
    ...
    if (it == semesters_.cend()) {
        ...
        nextCourseLabel_->setText(QStringLiteral("先新建学期，或导入 WakeUp 备份。"));
        return;
    }
...
void CourseSidebar::importWakeUp()
{
    const QString file = QFileDialog::getOpenFileName(this, QStringLiteral("导入 WakeUp 课程表备份"), {},
        QStringLiteral("WakeUp 课程表 (*.wakeup_schedule);;JSON 文件 (*.json);;所有文件 (*)"));
    if (file.isEmpty()) return;
    const auto parsed = WakeUpScheduleImporter::parseFile(file);
    if (!parsed.success) { QMessageBox::warning(this, QStringLiteral("导入失败"), parsed.error); return; }
    const QString summary = QStringLiteral("将新建学期“%1”，从 %2 开始，共 %3 周，包含 %4 条上课安排。\n\n导入后会切换到这个学期。")
        .arg(parsed.value.name, parsed.value.startDate.toString(Qt::ISODate))
        .arg(parsed.value.totalWeeks).arg(parsed.value.courses.size());
    if (QMessageBox::question(this, QStringLiteral("确认导入"), summary) != QMessageBox::Yes) return;
    const auto imported = repository_->importSchedule(parsed.value);
    if (!imported.success) { QMessageBox::warning(this, QStringLiteral("导入失败"), imported.error); return; }
    refresh(); emit scheduleChanged(); emit statusMessage(QStringLiteral("WakeUp 课表已经导入。"), false);
}
