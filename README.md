...
3. 首次运行会在 `%USERPROFILE%\.hyori` 创建配置和数据库。右键立绘打开“设置…”，填写自己的 AI 接口、模型和 API Key 后即可对话。
...
- 单实例运行保护、常驻系统托盘控制菜单和本地轮转运行日志
- 专注模式、番茄钟、背景视频与跨重启保存的本地对话记录窗口
...
- 独立透明待办窗口：支持标题、说明、状态、优先级、截止时间，以及新增、编辑和删除
- 全局主题：可选择跟随系统、冰织夜色或晨雾浅色，所有已打开窗口会同步切换
...
首次启动会生成一个默认配置。可以从立绘右键菜单打开“设置…”直接修改常用的 AI、TTS 与语音选项；也可以关闭程序后手动编辑配置文件。至少需要设置 `llmApiKey`：
...
- `theme`：全局窗口主题，可选 `system`、`night`、`mist`；也可在“设置 → 外观”中选择。
...
- 点击专注窗口顶部的“待办”，或右键立绘选择“打开待办事项”，会打开独立的透明待办窗口；它拥有自己的窗口焦点和任务栏入口，不会覆盖或拦截专注页。拖动顶部标题区域可以移动窗口；可管理待处理、进行中和已完成事项，并设置优先级与截止时间；点击右上角关闭或按 `Esc` 隐藏窗口。
- 在“设置 → 外观”选择主题并保存，桌宠气泡、专注页、透明待办、手账、周课表、对话记录及系统菜单会立即同步更新。
...
立绘模式目录中使用 `neutral.png`、`happy.png`、`shy.png`、`concerned.png`、`excited.png` 等以情绪命名的图片。默认模式缺少 `excited.png` 时会自动使用 `happy.png` 或 `smile.png`；其他缺失情绪会保持当前立绘不变。

## 已知边界与后续方向

当前版本仍是桌宠 MVP，GPT-SoVITS 服务需要单独启动。业务数据保存在 `~/.hyori/hyori.db`，配置保存在 `~/.hyori/config.json`，最近 500 条对话保存在 `~/.hyori/conversation-history.json`，运行日志位于 `~/.hyori/logs/`。
